...
MonocularMode::MonocularMode() :Node("mono_node_cpp")
{
    ...
    subexperimentconfigName = "/mono_py_driver/experiment_settings_to_common"; // topic that sends out some configuration parameters to the cpp ndoe
    pubconfigackName = "/mono_py_driver/exp_settings_ack"; // send an acknowledgement to the python node
    subImgMsgName = "/mono_py_driver/img_msg_to_common"; // topic to receive RGB image messages
    subTimestepMsgName = "/mono_py_driver/timestep_msg_to_common"; // topic to receive RGB image messages
    subBadPointsMsgName = "/bad_points"; // topic to receive RGB image messages
    ...
    subBadPointMsg_subscription_ = this->create_subscription<geometry_msgs::msg::PoseArray>(subBadPointsMsgName, 1, std::bind(&MonocularMode::BadPoint_callback, this, _1));
...
void MonocularMode::BadPoint_callback(const geometry_msgs::msg::PoseArray& msg)
{
    std::lock_guard<std::mutex> lock(mBadPointsMutex);
    badPoints.clear();

    for (const auto& pose : msg.poses)
    {
        badPoints.emplace_back(cv::Point2f(pose.position.x, pose.position.y));
    }

    RCLCPP_INFO(this->get_logger(), "Received %ld bad points.", msg.poses.size());
}
...
void MonocularMode::Img_callback(const sensor_msgs::msg::Image& msg)
{
    ...
    Sophus::SE3f Tcw = pAgent->TrackMonocular(cv_ptr->image, timeStep, badPoints); 
