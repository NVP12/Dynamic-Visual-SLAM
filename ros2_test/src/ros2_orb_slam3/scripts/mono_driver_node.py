...
class MonoDriver(Node):
    def __init__(self, node_name = "mono_py_node"):
        ...
        self.sub_exp_ack_name = "/mono_py_driver/exp_settings_ack_to_driver"
    ...
    def run_py_node(self, idx, imgz_name):
        ...
        timestep = timestep = float(imgz_name.replace(".png", "")) # Kept if you use a custom message interface to also pass timestep value
        self.frame_id = self.frame_id + 1  
        #print(img_look_up_path)
        # print(f"Frame ID: {frame_id}")

        # Based on the tutorials
        image = cv2.imread(img_look_up_path, cv2.IMREAD_GRAYSCALE)
        img_msg = self.br.cv2_to_imgmsg(image, encoding="mono8")
...
def main(args = None):
    ...
    rate = n.create_rate(5) # https://answers.ros.org/question/358343/rate-and-sleep-function-in-rclpy-library-for-ros2/
