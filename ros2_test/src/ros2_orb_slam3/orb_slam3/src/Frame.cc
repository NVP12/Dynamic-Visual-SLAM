...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor, ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, const std::vector<cv::Point2f>& badPoints, Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
     ...
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    if(!badPoints.empty())
    {
        float threshold = 5.0f; // You can adjust the threshold (in pixels)
        ...
        {
            bool reject = false;
            for (const auto& badPoint : badPoints)
            {
                float dx = mvKeys[i].pt.x - badPoint.x;
                float dy = mvKeys[i].pt.y - badPoint.y;
            ...
            if (!reject)
            {
                ...
                filteredDescriptors.push_back(mDescriptors.row((int)i));
            }
        }
