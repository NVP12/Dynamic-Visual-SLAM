...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const std::vector<cv::Point2f>& badPoints, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed, timestamp, badPoints, filename);
    ...
    // === Trajectory Logging for MONOCULAR SLAM ===
    if (mpTracker->mState == Tracking::OK)
    {
    Sophus::SE3f Twc = Tcw.inverse();  // Convert to world-to-camera
    Eigen::Vector3f t = Twc.translation();
    Eigen::Quaternionf q = Twc.unit_quaternion();

    static std::ofstream trajFile("MonocularTrajectory.txt", std::ios_base::out);
    ...
    trajFile << std::setprecision(6) << timestamp << " "
             << std::setprecision(9)
             << t.x() << " " << t.y() << " " << t.z() << " "
             << q.x() << " " << q.y() << " " << q.z() << " " << q.w()
             << std::endl;
